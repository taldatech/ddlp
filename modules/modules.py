...
class CNNDecoder(nn.Module):
    def __init__(self, cdim=3, channels=(64, 128, 256, 512, 512, 512), image_size=64, in_ch=16, n_kp=8,
        ...
        self.final_conv = ConvBlock(channels[0], cdim, kernel_size=1, bias=True,
                                    activation=False, batchnorm=False, use_resblock=False, pad_mode=pad_mode)
...
class ParticleFilterMixer(nn.Module):
    """
    Particle Filter Mixer: given n_kp_prior keypoints, extract patches, and use convolution and mlp-mixer-style
    to filter to n_kp_enc particles
    """

    def __init__(self, anchor_size, image_size, n_kp_prior, n_kp_enc, cnn_channels=(16, 16, 32), margin=0, ch=3,
                 use_resblock=False, hidden_dims=(256, 128)):
        super().__init__()
        ...
        # mixer
        self.conv = nn.Sequential(nn.Conv1d(in_channels=self.n_kp_prior, out_channels=hidden_dim_2, kernel_size=1),
                                  nn.ReLU(True),
                                  nn.Conv1d(in_channels=hidden_dim_2, out_channels=self.n_kp_prior, kernel_size=1))
        self.dense = nn.Sequential(nn.Linear(hidden_dim_2, hidden_dim_2),
                                   nn.ReLU(True),
                                   nn.Linear(hidden_dim_2, hidden_dim_2))
        self.pos_projection = nn.Linear(2, hidden_dim_2 // 2)
        self.conv_projection = nn.Linear(fc_in_dim, hidden_dim_2 // 2)

        self.mixer = nn.Sequential(PreNormResidual(hidden_dim_2, self.conv),
                                   PreNormResidual(hidden_dim_2, self.dense),
                                   nn.LayerNorm(hidden_dim_2),
                                   nn.Linear(hidden_dim_2, self.n_kp_enc))

        # attn
        # self.key_projection = nn.Linear(2, hidden_dim_2)
        # self.conv = nn.Sequential(nn.Conv1d(in_channels=hidden_dim_2, out_channels=hidden_dim_2, kernel_size=1),
        #                           nn.ReLU(True),
        #                           nn.Conv1d(in_channels=hidden_dim_2, out_channels=hidden_dim_2, kernel_size=1))
        # self.query_projection = nn.Sequential(nn.Linear(fc_in_dim + 2, hidden_dim_2),
        #                                       nn.ReLU(True),
        #                                       nn.Conv1d(in_channels=self.n_kp_prior, out_channels=hidden_dim_2, kernel_size=1),
        #                                       PreNormResidual(hidden_dim_2, self.conv),
        #                                       nn.LayerNorm(hidden_dim_2),
        #                                       nn.Linear(hidden_dim_2, self.n_kp_enc))
        # other mixer versions
        # self.mixer = nn.Sequential(nn.Linear(hidden_dim_2, hidden_dim_2),
        #                            nn.ReLU(True),
        #                            nn.Linear(hidden_dim_2, hidden_dim_2)
        #                            )
        #
        # self.to_kp = nn.Sequential(nn.Linear(hidden_dim_2, self.n_kp_enc * 2),
        #                            nn.Tanh())

        # self.mixer = nn.Sequential(nn.Conv1d(in_channels=self.n_kp_prior, out_channels=hidden_dim_2, kernel_size=1),
        #                            nn.ReLU(True),
    ...
    def quantize(self, query, kv):
        # query: [batch_size, n_kp_enc, 2]
        # kv: [batch_size, n_kp_prior, 2]
        batch_size, n_kp_enc, _ = query.shape
        n_kp_prior = kv.shape[1]
        dist = ((query.unsqueeze(2) - kv.unsqueeze(1)) ** 2).sum(-1)  # [bs, n_kp_enc, n_kp_prior]
        # _, embed_ind = dist.min(-1)
        _, embed_ind = (-dist).max(-1)
        batch_ind = torch.arange(batch_size, device=query.device)[:, None]
        quantized = kv[batch_ind, embed_ind]  # [bs, n_kp_enc, 2]
        diff = (quantized.detach() - query).pow(2).mean()
        return quantized + (query - query.detach())  # straight-through

    def forward(self, x, kp, z_scale=None):
        # x: [bs, ch, image_size, image_size]
        # kp: [bs, n_kp_prior, 2] in [-1, 1]
        # z_scale: [bs, n_kp, 2]
        batch_size, _, _, img_size = x.shape
        _, n_kp, _ = kp.shape
        assert n_kp == self.n_kp_prior, f"input kp doesn't match self.n_kp_prior: {self.n_kp_prior}"
        ...
        z_pos = kp.reshape(-1, kp.shape[-1]).detach()
        ...
        # mixer - original
        cropped_objects_flat = torch.cat([self.conv_projection(cropped_objects_flat),
                                          self.pos_projection(kp.detach())], dim=-1)  # [batch_size, n_kp, hidden_dim_2]
        mixer_out = self.mixer(cropped_objects_flat)  # [batch_size, n_kp_prior, n_kp_enc]

        mixer_out = torch.softmax(mixer_out, dim=1)
        kp_enc = kp.transpose(-2, -1) @ mixer_out  # [batch_size, 2, n_kp_enc]
        kp_enc = kp_enc.transpose(-2, -1)  # [batch_size, n_kp_enc, 2]

        # other mixer versions
        # mixer_out = self.mixer(cropped_objects_flat)  # [batch_size, n_kp_prior, hidden_dim_2]
        # mixer_out = mixer_out.mean(1)  # [batch_size, hidden_dim_2]
        # mixer_out = mixer_out.max(1)[0]  # [batch_size, hidden_dim_2]

        # quantization - requires latent loss
        # kp_enc = self.to_kp(mixer_out).view(mixer_out.shape[0], -1, kp.shape[-1])  # [batch_size, n_kp_enc, 2]
        # kp_enc = self.quantize(kp_enc, kp)

        # attention
        # cropped_objects_flat = torch.cat([cropped_objects_flat, kp.detach()], dim=-1)  # [batch_size, n_kp, in_dim +2]
        # kp_proj = self.key_projection(kp.detach())  # [bs, n_kp_prior, hidden_dim_2]
        # query_proj = self.query_projection(cropped_objects_flat).transpose(-2, -1)  # [bs, n_kp_enc, hidden_dim_2]
        # att = (query_proj @ kp_proj.transpose(-2, -1)) * (1.0 / math.sqrt(kp_proj.size(-1)))  # [bs, n_kp_enc, n_kp_prior]
        # att = F.softmax(att, dim=-1)
        # kp_enc = att @ kp  # [bs, n_kp_enc, 2]

        return kp_enc.contiguous()
...
class ParticleAttributeEncoder(nn.Module):
    ...
    def __init__(self, anchor_size, image_size, cnn_channels=(16, 16, 32), margin=0, ch=3, max_offset=1.0,
                 ...
                 hidden_dims=(256, 256), attn_dropout=0.1):
        super().__init__()
...
class ParticleFeaturesEncoder(nn.Module):
    ...
    def __init__(self, anchor_size, features_dim, image_size, cnn_channels=(16, 16, 32), margin=0, ch=3,
                 use_resblock=False, enable_attn=False, hidden_dims=(256, 256), attn_dropout=0.1):
        super().__init__()
        # use_correlation_heatmaps: use correlation heatmaps as input to model particle properties (e.g., xy offset)
        self.anchor_size = anchor_size
        self.channels = cnn_channels
